...
func TestPrepareForSigning_LenNotPax(t *testing.T) {
	...
	_, err := PrepareForSigning(ec, 0, 3, big.NewInt(42), ks) // pax=3 but len=2
	assert.Error(t, err)
...
func TestPrepareForSigning_IndexOutOfRange(t *testing.T) {
	...
	_, err := PrepareForSigning(ec, 2, 2, big.NewInt(42), ks) // i=2 >= len(ks)=2
	assert.Error(t, err)
...
func TestPrepareForSigning_DuplicateIndices(t *testing.T) {
	...
	_, err := PrepareForSigning(ec, 0, 2, big.NewInt(42), ks)
	assert.Error(t, err)
