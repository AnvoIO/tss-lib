...
// TestStoreMessageRejectsCrossCommitteeReplacement is the EdDSA counterpart of the
// ECDSA resharing regression test for the intra-session message-replacement guard.
//
// Resharing spans two independent, overlapping committee index spaces. A naive
// self-echo exemption of the form `fromPIdx == p.PartyID().Index` compares a single
// index against both spaces, so a new-committee party whose new index equals an
// old-committee peer's old index would mis-classify that peer as "self" (isDup=false)
// and silently accept a replacement of the peer's already-stored commitment. The
// guard instead detects self-echoes by sender IDENTITY (key).
//
...
func TestStoreMessageRejectsCrossCommitteeReplacement(t *testing.T) {
	...
	// Two disjoint committees, both index-0-based, so a new-committee party and an
	// old-committee party can share the numeric index 0 with distinct keys.
	...
	// 3. An identical re-send is still tolerated (idempotent at-least-once delivery
	//    must not be mistaken for adversarial replacement).
	dup := mkMsg(0xC0FFEE)
