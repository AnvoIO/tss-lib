...
func TestFac(test *testing.T) {
	...
	// An oversized response scalar must be rejected by the range check before
	// any modular exponentiation (anti-DoS). W1 is bounded by 2*q^3*NCap.
	q := ec.Params().N
...
// TestVerifyRejectsMalformedInputs asserts the group-membership invariants added
// to Verify: N0/NCap must be usable unknown-order moduli, s/t canonical
// generators of Z_NCap* and distinct, and the prover-supplied commitments
// P/Q/A/B/T must be units of Z_NCap*.
