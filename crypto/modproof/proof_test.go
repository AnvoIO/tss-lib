...
// prime factors P and Q are themselves safe primes. Keeping an executable
// example prevents release notes and call-site comments from silently growing
// a stronger guarantee than the verifier implements.
func TestProofScopeDoesNotAssertSafePrimeFactors(t *testing.T) {
	newNonSafeBlumPrime := func() *big.Int {
		for {
			...
			if p.Bit(1) != 1 { // require p == 3 mod 4
				continue
			}
