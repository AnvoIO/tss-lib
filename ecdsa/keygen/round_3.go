	...
	// 12-16. compute Xj for each Pj
	{
		...
		for j := 0; j < round.PartyCount(); j++ {
			...
			for c := 1; c <= round.Threshold(); c++ {
				z = modQ.Mul(z, kj)
				// z is kj^c mod q, nonzero for a well-formed party key; but a peer
				// whose KeyInt ≡ 0 (mod q) would drive Vc[c]^z to the point at
				// infinity and panic. Use the checked variant so it aborts with a
				// culprit instead, matching the Add error path below.
				vcz, mErr := Vc[c].ScalarMultChecked(z)
				if mErr != nil {
					culprits = append(culprits, Pj)
					continue
				}
				BigXj, err = BigXj.Add(vcz)
				if err != nil {
					culprits = append(culprits, Pj)
				}
			}
