...
import (
	...
	resharing "github.com/AnvoIO/tss-lib/v4/ecdsa/resharing"
...
func TestResharingRejectsSlotZeroPubKeySwap(t *testing.T) {
	...
	for j, pID := range newPIDs {
		params, perr := tss.NewReSharingParameters(ec, oldCtx, newCtx, pID, len(oldPIDs), threshold, newPCount, newThreshold)
		assert.NoError(t, perr, "should build resharing params")
