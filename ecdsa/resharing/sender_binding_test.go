...
func TestStoreMessageBindsNewCommitteeSenderByKey(t *testing.T) {
	...
	params, err := tss.NewReSharingParameters(
		tss.S256(), tss.NewPeerContext(oldIDs), tss.NewPeerContext(newIDs),
	...
	msg := NewDGRound2Message2(nil, claimedWrongIndex)
	ok, storeErr := party.StoreMessage(msg)
	require.True(t, ok)
	require.Nil(t, storeErr)
	require.Same(t, msg, party.temp.dgRound2Message2s[2])

	outsider := tss.NewPartyID("outsider", "outsider", big.NewInt(99))
	outsider.Index = 0
	ok, validationErr := party.ValidateMessage(NewDGRound2Message2(nil, outsider))
	require.False(t, ok)
